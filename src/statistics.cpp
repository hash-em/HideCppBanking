...
int statsticsInter() {
	string options[10] = { "Number of loans","Number of loans by type","Number of loans by status","Active loans in date range"
		,"Customer with highest number of loans","Customer with highest balance"
		,"Customer with lowest balance","Number of employees","Number of employees by branch","Exit"};
	printOptions(options, 10);
	int choice;
	cin >> choice;
	customerList customers = parseCustomers();
	EmployeeArray employees = loadEmployeesFromFile("../data/employees.json");
	switch (choice) {
		case 1: {
		int n = numberOfLoans(customers);
		cout << "Total number of loans: " << n << endl;
		break;
		}
		case 2: {
			string type;
			...
			cin >> type;
			int n = loansType(customers, type);
			cout << "Number of loans of type " << type << ": " << n << endl;
			break;
		}
		case 3: {
			string status;
			...
			cin >> status;
			int n = loansStatus(customers, status);
			cout << "Number of loans with status " << status << ": " << n << endl;
			break;
		}
		case 4: {
			string start, end;
			...
			cin >> end;
			int n = activeLoans(customers, start, end);
			cout << "Number of active loans between " << start << " and " << end << ": " << n << endl;
			break;
		}
		case 5: {
			customer cust = highestNbLoans(customers);
			cout << "Customer with highest number of loans: " << cust.name << " (Account #: " << cust.acc_num << ")" << endl;
			break;
		}
		case 6: {
			customer cust = highestBalance(customers);
			cout << "Customer with highest balance: " << cust.name << " (Account #: " << cust.acc_num << ", Balance: " << cust.balance << ")" << endl;
			break;
			  }	
		case 7: {
			customer cust = lowestBalance(customers);
			cout << "Customer with lowest balance: " << cust.name << " (Account #: " << cust.acc_num << ", Balance: " << cust.balance << ")" << endl;
			break;
		}
		case 8: {
			int n = numberOfEmployees(employees);
			cout << "Total number of employees: " << n << endl;
			break;
		}
		case 9: {
			string branch;
			...
			cin >> branch;
			int n = numEmployeesByBranch(employees, branch);
			cout << "Number of employees in branch " << branch << ": " << n << endl;
			break;
		}
		case 10{
			return 67;
