...
CompLoan newCompLoanList(){
    return CompLoan{nullptr,0};
}
TransactionHistory newHistoryList(){
    return TransactionHistory{nullptr,0};
}
...
{
    tranStack stack = newTranStack();
    string path = "../data/history.json";

    ifstream file(path, ios::binary);
    if (!file.is_open()) return stack;
...
{
    CompLoan completed = newCompLoanList();
    string key;
    ...
    while (*c)
    {
        ...
        if (*c == '{')
        {
            ...
            addCompletedLoan(completed, object);
        }
        c++;
    }
...
{
    CLoanNode *curr = clist.head;

    if (!curr)
    {
        clist.size++;
        clist.head = new CLoanNode{loan, nullptr};
        return;
    }
    ...
    CLoanNode *new_l = new CLoanNode{loan, nullptr};
    curr->next = new_l;
    clist.size++;
}
...
{
    CompLoan completed = newCompLoanList();
    string path = "../data/completed_loans.json";

    ifstream file(path, ios::binary);
    if (!file.is_open()) return completed;

    string buffer((istreambuf_iterator<char>(file)), istreambuf_iterator<char>());
    const char *c = buffer.c_str();

    return parseCompletedLoans(c);
}
...
void addHistoryTransaction(TransactionHistory &hist, const tran &t)
{
    ...
    {
        hist.size++;
        hist.head = new HistoryNode{t, nullptr};
        return;
    }
    ...
    HistoryNode *newNode = new HistoryNode{t, nullptr};
    curr->next = newNode;
    hist.size++;
}
TransactionHistory parseTransactionHistory(const char *&c)
{
    TransactionHistory hist = newHistoryList();
    string key;
        ...
        if (*c == '{')
        {
            ...
            while (*c && *c != '}')
            {
                ...
                if (*c == '"')
                {
                    ...
                    if (key == "id") t.id = parseStringValue(c);
                    else if (key == "account") t.acc_num = parseStringValue(c);
                    else if (key == "type") t.type = parseStringValue(c);
                    else if (key == "amount") t.amount = parseFloatValue(c);
                    else if (key == "date") t.date = parseStringValue(c);
                }
                c++;
            }

            addHistoryTransaction(hist, t);
        }
...
TransactionHistory parsetransactionhistory()
{
    ...
    ifstream file(path, ios::binary);
    if (!file.is_open()) return hist;

    string buffer((istreambuf_iterator<char>(file)), istreambuf_iterator<char>());
    const char *c = buffer.c_str();

    return parseTransactionHistory(c);
}